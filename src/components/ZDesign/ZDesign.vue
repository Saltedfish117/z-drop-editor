...
const pages = computed(() => {
	const p = selectCanvas.value.children?.filter(
		(item) => item.type === "page"
	);
...
const submitCreatePage = (close: () => void) => {
	...
	if (pages.value.length === 0) {
		layout = {
			...
			x: -selectCanvas.value.layout.x,
			y: -selectCanvas.value.layout.y,
		};
	} else {
						...
						<ZPopup>
							<template #default="{ close }">
								...
								>
									...
									<ZBtn
										color="primary"
										@click="submitCreateCanvas(close)"
										>确定</ZBtn
									>
								</div>
							</template>
						...
						<ZPopup>
							<template #default="{ close }">
								...
								>
									...
									<ZBtn
										color="primary"
										@click="removeCanvas(close, item.id)"
										>确定</ZBtn
									>
								</div>
							</template>
