...
withDefaults(defineProps<ZBtnProps>(), {
  color: "default",
  plain: false,
  round: false,
  disabled: false,
...
.ZBtn {
  position: relative;
  border: 1px solid rgba(var(--z-quiet));
  border-radius: 4px;
  color: var(--z-text);
...
.z-btn-link-default {
  ...
  position: relative;
  background: linear-gradient(to right, rgb(var(--z-text)), rgb(var(--z-text))) no-repeat
    right bottom;
...
@each $color in primary, success, danger, warning, info {
  .z-btn-#{$color} {
    border-color: rgba(var(--z-#{$color}), 1);
    background-color: rgba(var(--z-#{$color}), 1);
