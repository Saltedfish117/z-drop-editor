...
onMounted(() => {
  ...
  document.documentElement.addEventListener("scroll", closeMenu);
});
onUnmounted(() => {
  const events = ["contextmenu", "click"];
  events.forEach((event) => {
    parent.value.removeEventListener(event, handleContextMenu);
    document.documentElement.removeEventListener(event, handleClickOutside);
  });
  document.documentElement.removeEventListener("scroll", closeMenu);
});
</script>
<template>
  <div
