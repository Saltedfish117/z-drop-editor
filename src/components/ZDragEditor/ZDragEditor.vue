...
import type ZDragEditorCanvas from "../ZDragEditorCanvas";
defineOptions({
  name: "ZDragEditor",
});
...
const onDragEnd = (fn: () => void, id: string) => {
  const nodeId = id;
  if (!nodeId) throw new Error("nodeId in instance attrs not found");
  const drag = dragMap.get(nodeId);
